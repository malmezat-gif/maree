globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/page-6WSfFx2p.js","assets/pwa-registration-Bzi7kymf.js","assets/viewport-fit-D8xBvzt4.js","assets/layout-segment-context-CWM_tH4Z.js"];
...
function getDraftSecret() {
	return "233b1ec5-bbe0-4595-ab94-412878db721e";
}
/**
...
var NextRequest = class extends Request {
	...
	constructor(input, init) {
		...
		this._url = {}.__NEXT_NO_MIDDLEWARE_URL_NORMALIZE ? url.toString() : this._nextUrl.toString();
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "ecf16b9c-9e4d-41a3-a569-a8a9c34e82ce";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "ecf16b9c-9e4d-41a3-a569-a8a9c34e82ce") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "ecf16b9c-9e4d-41a3-a569-a8a9c34e82ce",
		rootBoundaryId
	});
}
/**
...
async function dispatchAppPageInner(options) {
	...
	return renderAppPageLifecycle({
		...
		isForceDynamic,
		isForceStatic,
		isPrerender: {}.VINEXT_PRERENDER === "1",
		isProduction: options.isProduction,
		isRscRequest: options.isRscRequest,
		isrDebug: options.isrDebug,
		isrHtmlKey: options.isrHtmlKey,
...
var PwaRegistration = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'PwaRegistration' is called on server");
}, "2e0fea4f4a63", "PwaRegistration");
//#endregion
...
*/
var ViewportFit = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'ViewportFit' is called on server");
}, "7460d4986a50", "ViewportFit");
//#endregion
...
var title = "Marée — La mer au fil du temps";
var description = "Consultez les horaires et coefficients de marée, puis explorez la hauteur de l’eau au fil de la journée.";
async function generateMetadata() {
	...
	return {
		...
		icons: {
			icon: [{
				url: "/icons/maree-192.png",
				sizes: "192x192",
				type: "image/png"
			}, {
				url: "/icons/maree-512.png",
				sizes: "512x512",
				type: "image/png"
			}],
			apple: [{
...
function RootLayout({ children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("html", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("body", { children: [
			children,
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ViewportFit, {}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(PwaRegistration, {})
		] })
	});
}
...
//#endregion
//#region lib/tides-api.ts
var API_BASE_URL = "https://api-maree.fr";
var TIMEZONE = "Europe/Paris";
var STEP_MINUTES = 10;
var MAX_LEVEL_POINTS = 168 * (60 / STEP_MINUTES);
var DEFAULT_TIMEOUT_MS = 8e3;
var DAY_MS = 864e5;
var SUCCESS_CACHE_CONTROL = "public, max-age=300, s-maxage=21600, stale-while-revalidate=86400";
var ERROR_CACHE_CONTROL = "no-store";
var TIDE_PORTS = {
	biarritz: {
		id: "biarritz",
		name: "Biarritz",
...
var UpstreamTimeoutError = class extends Error {
	constructor() {
		super("Upstream request timed out");
		this.name = "UpstreamTimeoutError";
	}
};
var UpstreamPayloadError = class extends Error {
	constructor() {
...
var SOURCE = {
	id: "api-maree.fr",
	attribution: "Données de marée fournies par api-maree.fr sous licence CC BY, calculées à partir de composantes harmoniques Ifremer / PREVIMER, elles-mêmes sous licence CC BY.",
	license: "CC BY 4.0",
	official: false,
	navigation: false
};
async function handleTidesRequest(request, dependencies = {}) {
	try {
		const query = parseQuery(request, dependencies.now ?? /* @__PURE__ */ new Date());
		if (query.port.siteId === null) throw new ApiError("unsupported_port", "Capbreton n’est pas encore disponible auprès de la source de données.", 422);
		const apiKey = dependencies.apiKey?.trim();
		if (!apiKey) throw new ApiError("service_unconfigured", "Le service de marée n’est pas encore configuré.", 503);
		...
		if (error instanceof UpstreamTimeoutError) return errorResponse("upstream_timeout", "La source de données met trop de temps à répondre.", 504);
		if (error instanceof UpstreamHttpError && error.status === 429) return errorResponse("upstream_rate_limited", "La source de données est momentanément très sollicitée.", 503, { "retry-after": "60" });
		return errorResponse("upstream_unavailable", "Les données de marée sont momentanément indisponibles.", 502);
	}
}
function parseQuery(request, now) {
	...
	if (!Object.prototype.hasOwnProperty.call(TIDE_PORTS, portId)) throw new ApiError("invalid_port", "Le port demandé ne fait pas partie des ports disponibles.", 400);
	const startDay = parseStrictDate(start);
	if (startDay === null) throw new ApiError("invalid_request", "Le paramètre start doit être une date au format YYYY-MM-DD.", 400);
	if (!/^[1-7]$/.test(daysValue)) throw new ApiError("invalid_request", "Le paramètre days doit être un entier compris entre 1 et 7.", 400);
	const days = Number(daysValue);
	const endInclusiveDay = startDay + days - 1;
	const today = parseStrictDate(dateInParis(now));
	if (today === null) throw new ApiError("upstream_unavailable", "La période demandée ne peut pas être calculée.", 502);
	if (startDay < today - 30 || endInclusiveDay > today + 30) throw new ApiError("date_out_of_range", "La période doit rester comprise entre 30 jours avant et 30 jours après aujourd’hui.", 422);
	return {
		port: TIDE_PORTS[portId],
		start,
...
function requiredSingleParam(params, name) {
	const values = params.getAll(name);
	if (values.length !== 1 || values[0] === "") throw new ApiError("invalid_request", `Le paramètre ${name} est obligatoire et ne doit apparaître qu’une fois.`, 400);
	return values[0];
}
...
async function fetchProviderData(query, apiKey, options) {
	...
	try {
		const [extrema, levels] = await Promise.all([fetchJson(options.fetch, extremaUrl, controller.signal), fetchJson(options.fetch, levelsUrl, controller.signal)]);
		return {
			extrema,
			levels
		};
...
function providerUrl(pathname, params) {
	const url = new URL(pathname, API_BASE_URL);
	url.search = new URLSearchParams(params).toString();
	return url;
}
async function fetchJson(fetcher, url, signal) {
	let response;
	try {
...
function normalizeExtrema(payload, query) {
	...
	for (const rawDay of root.data) {
		...
		if (typeof day.date !== "string" || !Array.isArray(day.extrema)) throw new UpstreamPayloadError();
		...
		for (const rawExtremum of day.extrema) {
			...
			if (extremum.type !== "PM" && extremum.type !== "BM" || typeof extremum.time !== "string" || !isStrictTime(extremum.time) || !isFiniteNumber(extremum.height)) throw new UpstreamPayloadError();
			normalized.push({
				date: day.date,
				time: extremum.time,
				type: extremum.type === "PM" ? "high" : "low",
				heightM: extremum.height,
				coefficient: extremum.type === "PM" && isFiniteNumber(extremum.coef) ? Math.round(extremum.coef) : null
			});
		}
	}
	return normalized.sort((left, right) => `${left.date}T${left.time}`.localeCompare(`${right.date}T${right.time}`));
}
function normalizeLevels(payload, query) {
	const root = requireRecord(payload);
	if (!Array.isArray(root.data)) throw new UpstreamPayloadError();
	...
	for (const rawPoint of root.data) {
		...
		if (typeof point.time !== "string" || !isFiniteNumber(point.height)) throw new UpstreamPayloadError();
...
function requireRecord(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new UpstreamPayloadError();
	return value;
}
...
function errorResponse(code, message, status, headers) {
	return jsonResponse({ error: {
		code,
		message
	} }, status, ERROR_CACHE_CONTROL, headers);
}
function jsonResponse(body, status, cacheControl, additionalHeaders) {
...
async function GET(request) {
	...
	return handleTidesRequest(request, { apiKey: env.API_MAREE_KEY });
}
//#endregion
...
var __isrDebug = {}.NEXT_PRIVATE_DEBUG_CACHE ? console.debug.bind(console, "[vinext] ISR:") : void 0;
var __classDebug = {}.VINEXT_DEBUG_CLASSIFICATION ? function(layoutId, reason) {
...
var routes = [{
	...
	ids: {
		"route": "route:/api/tides",
		"page": null,
		"routeHandler": "route-handler:/api/tides",
		"rootBoundary": "root-boundary:/",
		"layouts": ["layout:/"],
		"templates": [],
		"slots": {}
...
var __publicFiles = new Set([
	...
	"/assets/coast/coast-v3.webp",
	"/assets/coast/seabed-v1.webp",
	"/file.svg",
	"/globe.svg",
	"/icons/maree-1024.png",
	"/icons/maree-192.png",
	"/icons/maree-512.png",
...
var worker_entry_default = { async fetch(request, env, ctx) {
	...
	if (typeof HTMLRewriter === "undefined") {
		...
		return new Response(html.replace(/(<meta[^>]*name="viewport"[^>]*content=")([^"]*)(")/i, "$1width=device-width, initial-scale=1, viewport-fit=cover$3"), {
			status: response.status,
			statusText: response.statusText,
			headers: response.headers
		});
	}
	return new HTMLRewriter().on("meta[name=\"viewport\"]", { element(element) {
